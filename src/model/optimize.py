...
class OtimizadorONNX:
    ...
    def prever_onnx(self, texto: str) -> Dict:
        ...
        return {
            'classe': predication[0][0],
            'probabilidades': predication[1][0]
        }
...
class BenchmarkLatência:
    ...
    def __init__(
        self,
        caminho_modelo: str,
        caminho_vetorizador: str,
        caminho_onnx: str = None
    ):
        """
    ...
    def _preparar_batch(self, textos: list, tamanho_batch: int = 32) -> Tuple[np.ndarray, int]:
        ...
        """
        textos_processados = [
            self.preprocessador.preprocessar(texto) 
            for texto in textos
        ]
    ...
    def benchmark_comparativo(self, textos: list, iteracoes: int = 100) -> Dict:
        ...
        if self.sessao_onnx:
            ...
            # Calcula melhoria
            melhoria_pct = ((resultado_sklearn['media_ms'] - resultado_onnx['media_ms']) 
