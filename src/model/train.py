...
class ClassificadorTextoMedico:
    ...
    def _preparar_dados(self, textos: list, labels: list = None) -> Tuple[np.ndarray, np.ndarray]:
        ...
        if labels is None:
            ...
            X = (
                self.vetorizador.fit_transform(textos_processados)
                if not hasattr(self.vetorizador, "vocabulary_")
                else self.vetorizador.transform(textos_processados)
            )
    ...
    def treinar(
        ...
        logger.info(
            classification_report(
                ...
                y_pred,
                target_names=[
                    self.mapa_labels_inverso[i] for i in sorted(self.mapa_labels_inverso.keys())
                ],
            )
        )
    ...
    def salvar(self, caminho_modelo: str, caminho_vetorizador: str):
        ...
        joblib.dump(
            self.mapa_labels_inverso,
            caminho_modelo.replace(".pkl", "_mapa_labels_inverso.pkl"),
        )
    ...
    def carregar(cls, caminho_modelo: str, caminho_vetorizador: str) -> "ClassificadorTextoMedico":
        ...
        instancia.mapa_labels_inverso = joblib.load(
            caminho_modelo.replace(".pkl", "_mapa_labels_inverso.pkl")
        )
